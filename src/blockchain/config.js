...
const loadABI = async () => {
    const abiPath = path.join(__dirname, 'abi', 'PaymentTransactionStorage.json');
    try {
        const abiFile = await readFile(abiPath, 'utf-8');
        // Jika ABI berupa array, langsung kembalikan
        if (Array.isArray(JSON.parse(abiFile))) {
            return JSON.parse(abiFile);
        }
        // Jika ABI dalam format Truffle (mempunyai property abi)
...
const PaymentTransactionStorageABI = await loadABI();
const { web3, account } = initializeWeb3();

export const paymentContract = new web3.eth.Contract(
    PaymentTransactionStorageABI,
    process.env.CONTRACT_ADDRESS
);
